...
- 主選單以數位相框取代一起聽，可開啟全螢幕本機照片輪播（尚未選片時為空白背景），並顯示時鐘、歌曲資訊、音樂播放控制、上一張／下一張照片、上一首／下一首及相框內設定。放大後的控制層以第一個可換行列依序顯示時鐘、歌名與歌手，第二個可換行列集中所有操作按鈕，橫式與直式畫面都能清楚閱讀。按需載入的 MediaStore 瀏覽器是唯一的照片選取入口，可分頁多選系統已建立索引的內部儲存與 USB／SD 照片，也可依索引目錄瀏覽並一次選取或取消整個目錄，不會在 App 啟動時掃描，且會依系統語言完整顯示繁體中文。按需開啟的儲存裝置診斷會顯示 Android SDK、系統儲存裝置名稱、MediaStore volume 名稱、可取得時的掛載狀態／路徑／UUID，以及已索引照片數量，協助判斷車機 USB 索引問題，不會掃描或複製照片；已移除在車機上不可靠的系統檔案與 SAF 資料夾選擇功能，舊版本已保存的來源仍可讀取。可使用橫式拉桿選擇 5／10／15／30／60 秒輪播間隔，並設定完整顯示／裁切填滿，重新連接隨身碟後可重新掃描。原始檔案不會被複製或刪除；瀏覽器縮圖不寫入共用快取，僅在相框或瀏覽器可見時載入圖片。請於停車時使用，不在 Android Auto 顯示照片。一起聽仍可從設定 → 整合或選用的頂部工具列捷徑開啟。
- 將播放島放大 2 倍，提升資訊可見度。
