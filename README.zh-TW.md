...
- 主選單以數位相框取代一起聽，可開啟全螢幕本機照片輪播（尚未選片時為空白背景），並顯示時鐘、歌曲資訊、上一首／下一首及相框內設定。支援加入單張照片、遞迴掃描裝置或 USB 資料夾、設定 5–60 秒間隔及完整顯示／裁切填滿，重新連接隨身碟後可重新掃描。原始檔案不會被複製或刪除，失效來源可重新選取；僅在相框可見時載入圖片。請於停車時使用，不在 Android Auto 顯示照片。一起聽仍可從設定 → 整合或選用的頂部工具列捷徑開啟。
- 將播放島放大 2 倍，提升資訊可見度。
