...
- Photo frame replaces Listen Together in the main menu. It opens a full-screen local-photo slideshow (or a blank background before selection) with clock, song information, music playback controls, Previous/Next photo controls, and in-frame settings. Controls wrap automatically on narrow portrait screens. Add individual photos or recursively scanned device/USB folders through the system file picker; picker access is retained before scanning begins. Choose 5–60 second intervals and fit/crop, and rescan after reconnecting a drive. Original files are never copied or deleted; unavailable sources can be selected again. Image loading only runs while the frame is visible. Use while parked; photos are not shown in Android Auto. Listen Together remains available under Settings → Integrations and through its optional top-bar shortcut.
