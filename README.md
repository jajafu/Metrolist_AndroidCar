...
- Automatic queue continuation retries temporary failures with bounded backoff, waits for network recovery without duplicate requests, and offers a manual retry after the final failure. When a YouTube continuation ends, enabled similar content starts a new radio from the current tail song so playback can keep extending beyond the first 99 tracks without duplicating queued songs.
