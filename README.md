...
- Photo frame replaces Listen Together in the main menu. It opens a full-screen local-photo slideshow (or a blank background before selection) with clock, song information, music playback controls, Previous/Next photo controls, and in-frame settings. Controls wrap automatically on narrow portrait screens. Its on-demand MediaStore browser pages through indexed internal and USB/SD photos for multi-selection without startup scanning; browse indexed folders and select or deselect an entire folder at once. The system file picker and recursive device/USB folder picker remain available as compatibility fallbacks. Choose 5–60 second intervals and fit/crop, and rescan after reconnecting a drive. Original files are never copied or deleted; unavailable sources can be selected again. Browser thumbnails bypass shared caches, and image loading only runs while the frame or browser is visible. Use while parked; photos are not shown in Android Auto. Listen Together remains available under Settings → Integrations and through its optional top-bar shortcut.
